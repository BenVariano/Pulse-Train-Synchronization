...
Loop
        ...
        rcall  MySubroutine       ;Call Subroutine to write output to channel 1
...
LoopTime
        ...
        bcf  INTCON,GIEH        ;clear
        movff  TMR0L,TMR0LCOPY  ;Read 16-bit counter at this moment
        movff  TMR0H,TMR0HCOPY  ;write H -> copy (like above)
        movlw  low  Bignum      ;moveliteral value to wreg
        addwf  TMR0LCOPY,F      ;ad wreg  to Lcopy, store in F
        movlw  high  Bignum     ;move literal value to wreg
        addwfc  TMR0HCOPY,F     ;add wreg and Hcopy and carry bit, storing result in F
        movff  TMR0HCOPY,TMR0H  ;write Hcopy to TMR0H
        movff  TMR0LCOPY,TMR0L  ;Write 16-bit counter at this moment from lcopy to TMR0l
        movf  INTCONCOPY,W      ;Restore GIEH interrupt enable bit
        andlw  B'10000000'      ;and literal value into wreg
        iorwf  INTCON,F         ; inclusive or wreg with intcon, store in F
